...
from models import MetaData, engine, session, Table
...
class QueryExecutor:
    ...
    def _parse_conditions(table, condition_str):
        """
        Parse the WHERE condition into SQLAlchemy filters.
        """
        filters = []
    ...
    def _format_row(row, table):
        """
        Format a row to a dictionary with proper types for `datetime` and `Decimal`.
        """
        formatted_row = {}
...
if __name__ == "__main__":
    querry_result = QueryExecutor.execute_query(f"delete from complex_table where id = 4")
    print(querry_result)